...
import { normalizeDirection } from "@/lib/presentation";
...
export function getDirectionFromMockProduct(product: Product): RecommendationDirection {
  if (product.score >= 84 && product.marginScore >= 70 && product.riskScore <= 35) {
    return "Rocket Growth + PB" as RecommendationDirection;
  }
  if (product.score >= 75 && product.riskScore <= 45) {
    return "Rocket Growth" as RecommendationDirection;
  }
  if (product.marginScore >= 65 && product.riskScore <= 55) {
    return "PB" as RecommendationDirection;
  }
  if (product.score >= 60) {
    return "继续观察" as RecommendationDirection;
  }
  return "放弃" as RecommendationDirection;
}
