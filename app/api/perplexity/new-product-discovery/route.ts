...
export async function GET(request: NextRequest) {
  const result = await runPerplexityCapability("new-product-discovery", getFormat(request));
...
export async function POST(request: NextRequest) {
  ...
  const result = await runPerplexityCapability("new-product-discovery", format);
  return NextResponse.json(result);
}
